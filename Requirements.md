...
## 1.1 Назначение
Документ разработан для однозначной трактовки требований между разработчиком и заказчиком. Необходимо разработать web-сервис с простым функционалом, который будет представлять собой обширную базу данных природных и техногенных катастроф, произошедших с начала 20-го века во всем мире.  В этом документе описаны функциональные и нефункциональные требования к приложению «Watch & buy» для Android, IOS. Этот документ предназначен для команды, которая будет реализовывать и проверять корректность работы приложения.
